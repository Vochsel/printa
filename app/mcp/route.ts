...
function createServer(origin: string) {
  const server = new McpServer(
    ...
    {
      instructions:
        `Create ready-to-print geometry with create_procedural_model or use create_extruded_text for the focused text workflow. Printa Spec 1.0 composes primitive, custom-curve extrusion, imported SVG/PDF vector artwork, revolve, text, cellular lattice, volumetrically remeshed organic growth, water, fluid, and cloth sources with ordered modifiers including Catmull-Clark/Loop subdivision, Voronoi texture or smooth open wire shells, seeded branching surface vines, transformed arrays, and inset contour steps. Assemblies, repeats, and transforms are supported. JSON and YAML are accepted. If the MCP app is unavailable, encode the same JSON spec into ${origin}/make/model.stl?spec= as documented at ${origin}/skills.`,
    },
  );
  ...
  registerAppTool(
    ...
    "create_procedural_model",
    {
      ...
      description: "Validate and build a Printa Spec 1.0 document supplied as JSON or YAML, then show the result as an interactive 3D model with STL download. Use sources for primitives, custom Bézier extrusion, imported SVG/PDF outlines via the vector source, profile revolution, text, cellular lattices, smooth volumetric organic branching growth, water, fluid, or cloth simulation. Subdivide modifiers add Catmull-Clark, Loop, or linear topology; Voronoi modifiers support displaced cells, raised ridges, or wire mode for an open polywired and remeshed cellular shell; vine modifiers grow seeded branching rounded tendrils up a host as one watertight relief mesh. Revolved profiles support a global radius offset. Compose ordered modifiers, transformed arrays, inset contour steps, merge assemblies, or repeat transformed nodes.",
      inputSchema: {
