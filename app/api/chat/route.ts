...
source types: primitive (box|cylinder|cone|sphere|torus), revolve (a [radius,height] profile spun into a vase/bowl), extrude (a 2D path pulled up), text (any Google font, extruded), cellular (a seeded lightweight Voronoi-style strut lattice), organic (deterministic branching coral growth), water (ripple sim), fluid (an SPH liquid poured from above that pools over the other shapes in the scene), cloth (fabric draped over the other shapes). fluid and cloth are simulations: put them in an assembly alongside the solid shape(s) they should collide with, and the person runs them with the Simulate button.
modifiers (applied top→bottom, may stack): twist, taper, radialWave (flutes), axialWave (ripples up the height), bend, noise (roughen), voronoi (cell centers or boundary ridges), array (incrementally transformed copies), step (constant inset contour layers), smooth. Two simulation modifiers turn a shape's own geometry into a sim (run on the Simulate button, and collide with the other shapes in the scene): drape (settle the shape like cloth — great for draping text/shapes over another shape) and melt (melt the shape into a puddle). Both take a "frames" count that controls how far the sim runs.
