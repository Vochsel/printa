...
modifiers (applied top→bottom, may stack): twist, taper, radialWave (flutes), axialWave (ripples up the height), bend, noise (roughen), voronoi (cell centers, raised ridges, or "wire" mode that replaces the input with a smooth open cellular shell), array (incrementally transformed copies), step (constant inset contour layers), smooth. In Voronoi wire mode, amplitude is the wire radius and scale is the average cell size. Two simulation modifiers turn a shape's own geometry into a sim (run on the Simulate button, and collide with the other shapes in the scene): drape (settle the shape like cloth — great for draping text/shapes over another shape) and melt (melt the shape into a puddle). Both take a "frames" count that controls how far the sim runs.
