...
async function searchGoogle(query: string, key: string, signal?: AbortSignal): Promise<PlaceSearchHit[]> {
  const url = new URL(GOOGLE_GEOCODE);
  url.searchParams.set("address", query);
  url.searchParams.set("key", key);

  const response = await fetch(url, { signal, next: { revalidate: 86_400 } });
  ...
  return (body.results ?? []).slice(0, 6).flatMap((result) => {
    ...
    return [{ label: shorten(result.formatted_address ?? query), lat, lng, radiusM: radiusFor(bounds) }];
  });
}

async function searchNominatim(query: string, signal?: AbortSignal): Promise<PlaceSearchHit[]> {
...
export type PlaceSearchResult = { results: PlaceSearchHit[]; source: "google" | "openstreetmap" };

export async function searchPlaces(query: string, signal?: AbortSignal): Promise<PlaceSearchResult> {
  const trimmed = query.trim();
  if (trimmed.length < 2) return { results: [], source: "openstreetmap" };
  ...
  if (key) {
    try {
      const results = await searchGoogle(trimmed, key, signal);
  ...
  try {
    return { results: await searchNominatim(trimmed, signal), source: "openstreetmap" };
  } catch (error) {
    failures.push(error instanceof Error ? error.message : "Nominatim failed");
    throw new Error(failures.join(" · ") || "Place search is unavailable.");
  }
}
